...
- [x] **T8 — `tests/test_package_imports.py`.** Importa `wowrag` y cada
  subpaquete placeholder; afirma que no lanzan. _(Cubre R1, R2)_

- [x] **T9 — `tests/test_config.py`.**
...
- [x] **T11 — `tests/test_requirements_pinned.py`.** Lee `requirements.txt`;
...
- [x] **T12 — `tests/test_repo_hygiene.py`.** Afirma que `.gitignore` contiene
  `.env`, `.venv` y `__pycache__`, y que existe `.env.example` que menciona los
  campos de R5. _(Cubre R14)_

## Cierre

- [x] **T13 — Verificación.** Ejecutar `./init.sh` y confirmar exit 0 con la
